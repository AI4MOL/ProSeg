...
def main():
    ...
    # log & model folder
    if args.save_path == '':
        opt.MODEL_DIR += args.model_name + str(args.kl).replace(".","")+ '_{}_{}'.format(opt.DATASET, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    else:
    ...
    for fold_idx in range(opt.KFOLD):
        ...
        net = init_model(args, opt, kl=args.kl)
        ...
        for epoch in range(epoch_start, epochs):
            ...
            for step, (patch, masks, _) in enumerate(tqdm(train_loader)):
                ...
                writer.add_scalar('Loss/train_fold_idx-{}'.format(fold_idx), loss.item(), batches_done)
            ...
            metrics_dict = validate(net, test_loader, opt, writer, epoch, all_metrics=True)
            print_str = ''
    ...
    import subprocess
    command = f"python evaluate.py --model_name {args.model_name} --save_path {eval_dir}"
