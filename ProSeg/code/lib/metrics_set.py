...
def distance_one(x, y):
    ...
    per_class_iou = iou(x, y, axis=-1)
    return 1 - per_class_iou
...
def generalized_energy_distance(labels, preds, thresh=0.5, num_classes=2, all=False):
    #print("################", labels.shape, preds.shape)
    ################ stage 2 torch.Size([1, 4, 19, 128, 128]) torch.Size([1, 4, 19, 128, 128])
    ################ stage 1  torch.Size([1, 4, 31, 128, 128]) torch.Size([1, 10, 31, 128, 128])
...
def dice_at_all(labels, preds, thresh=0.5, is_test=False):
    #print("################", labels.shape, preds.shape)
    # stage 1 ################ torch.Size([1, 4, 28, 128, 128]) torch.Size([1, 10, 28, 128, 128])
    pred_masks = (preds > thresh).float()
    dice_each = []
    dice_max = []
    dice_max_reverse = []
