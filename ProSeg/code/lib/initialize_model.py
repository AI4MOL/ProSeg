...
from pionono_models.model_pionono_mix import PiononoModelMix
from pionono_models.model_pionono_prob_5143 import PiononoModelProb

def init_model(args, opt, kl=None, sigma=2, latent_dim=8):
    ...
    elif args.model_name == 'pionono_mix':
        if kl is None:
            kl = 0.1
            
        model = PiononoModelMix(input_channels=opt.INPUT_CHANNEL, num_classes=opt.OUTPUT_CHANNEL,
                             annotators=[0,1,2,3],
                             gold_annotators=0,
    ...
    elif args.model_name == 'pionono_prob':
        if kl is None:
            kl = 0.1
            
        model = PiononoModelProb(input_channels=opt.INPUT_CHANNEL, num_classes=opt.OUTPUT_CHANNEL,
                             annotators=[0,1,2,3],
                             gold_annotators=0,
                             latent_dim=latent_dim,
                             no_head_layers=3,
