...
def evaluate(net, test_loader, opt, result_path, sample_latent=False):
    ...
    with torch.no_grad():
        for test_step, sample in enumerate(tqdm(test_loader)):
            ...
            for idx in range(patch.shape[2]):
                if sample_latent:
                    output_slice = net.test_step(patch[:,:,idx], sample_num=opt.VAL_NUM).unsqueeze(2)
                else:
...
if __name__ == '__main__':
    ...
    metrics_dict = evaluate(net, test_loader, opt, result_path, sample_latent = args.sample)
