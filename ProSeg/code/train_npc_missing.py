...
def main():
    ...
    parser.add_argument("--model_name", type=str, default='pionono_prob') #[prob_unet, cm_global, cm_pixel, pionono]
    ...
    # log & model folder
    if args.save_path == '':
        opt.MODEL_DIR += args.model_name + str(args.kl).replace(".","")+ '_' + str(args.sigma).replace(".","") +'_{}_{}'.format(opt.DATASET, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    else:
    ...
    train_loader = DataLoader(db_train, batch_size=64, shuffle=True, num_workers=8, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch in range(epoch_start, epochs):
        ...
        # validate each epoch
        metrics_dict = validate(net, val_loader, opt, writer, epoch, all_metrics=True)
        print_str = ''
    ...
    import subprocess
    command = f"python evaluate_npc.py --model_name {args.model_name} --save_path {eval_dir}"
    # 执行命令
